...
Runner::Runner(vector<Graph *> &db, int ms, string target, int max_iter) {
    graph_db = db;
    graph_count = db.size();
    minsup = ms;
    target_code = target;
    ...
    for (int i = 0; i < num_candidates; i++) {
        ...
        if (current_adj.isCam()) {
            ...
            while (it != new_candidates.end()) {
                ...
                } else if (it->genCode() == target_code) {
                    return;
                } else {
                    it++;
                }
            }
            count += new_candidates.size();
            if (max_iteration < 0 || count < max_iteration) {
...
void Runner::displayOutput() const {
    cout << "Find status: " << (status ? "TRUE" : "FALSE") << endl;
